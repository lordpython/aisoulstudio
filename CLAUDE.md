...
- **Per-shot narration**: When the shot-breakdown agent doesn't populate `shot.scriptSegment`, `narrateAllShots` uses `voiceoverSplitter.ts` to distribute the scene voiceover across shots by sentence boundaries (weighted by `durationEst`). Splits on `.!?؟۔` to support Arabic/Urdu and preserves inline delivery markers (`[emphasis]`, `[pause: beat]`, `[breath]`, `[slow]`) intact within each chunk.
- **Delivery markers**: TTS normalization happens in `audio-processing/deliveryMarkers.ts` before sending text to Gemini TTS — keep marker syntax stable across `narratorService` and any new pipeline adding scripted narration.
